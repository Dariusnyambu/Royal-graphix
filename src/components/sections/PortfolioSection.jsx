...
export default function PortfolioSection({ items = [], loading = false, preview = false }) {
  ...
  const allCategories = ['All', ...Array.from(new Set([
    ...PORTFOLIO_CATEGORIES.filter(c => c !== 'All'),
        ...
        {selected && (
          <div>
            <div style={{ height: 260, borderRadius: 'var(--radius-md)', overflow: 'hidden', marginBottom: '1.5rem' }}>
              <img src={selected._img} alt={selected.image_alt || `${selected.title} — ${selected.category} project by Royal Graphix`} loading="lazy" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            </div>
            <span style={{
            ...
            {(selected.client_problem || selected.solution) && (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1.25rem', marginBottom: '1.5rem' }}>
                {selected.client_problem && (
                  <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', padding: '1.1rem' }}>
                    <div style={{ fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--text-muted)', marginBottom: '0.5rem' }}>The Challenge</div>
                    <p style={{ fontSize: '0.87rem', color: 'var(--text-muted)', lineHeight: 1.65 }}>{selected.client_problem}</p>
                  </div>
                )}
                {selected.solution && (
                  <div style={{ background: 'rgba(200,16,46,0.05)', border: '1px solid rgba(200,16,46,0.15)', borderRadius: 'var(--radius-md)', padding: '1.1rem' }}>
                    <div style={{ fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--red)', marginBottom: '0.5rem' }}>Our Solution</div>
                    <p style={{ fontSize: '0.87rem', color: 'var(--text-muted)', lineHeight: 1.65 }}>{selected.solution}</p>
                  </div>
                )}
              </div>
            )}
            {selected.gallery_images?.length > 0 && (
              <div style={{ display: 'flex', gap: '0.6rem', overflowX: 'auto', marginBottom: '1.5rem', paddingBottom: 4 }}>
                {selected.gallery_images.map((src, i) => (
                  <img key={i} src={src} alt={`${selected.title} ${i + 2}`} style={{ width: 84, height: 84, objectFit: 'cover', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border)', flexShrink: 0 }} />
                ))}
              </div>
            )}
            <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
              {selected.slug && (
                <Link to={`/portfolio/${selected.slug}`} style={{ textDecoration: 'none' }}>
                  <button style={{
  ...
  return (
    ...
    >
      ...
      <div className="img-zoom" style={{ position: 'relative', height: 230, overflow: 'hidden' }}>
        <img
          src={project._img}
          alt={project.image_alt || `${project.title} — ${project.category} project by Royal Graphix`}
