...
import { Image, MessageSquare, FileText, Star, TrendingUp, Plus, ArrowRight, Briefcase, FileInput, Radio } from 'lucide-react'
...
export default function Dashboard() {
  const { user }                   = useAuth()
  const { items: portfolio }       = useAdminPortfolio()
  const { contacts }               = useContacts()
  const { posts: blogPosts }       = useAdminBlog()
  const { reviews }                = useAdminReviews()
  const { liveNow }                = useAnalytics(7)
  ...
  const stats = [
    ...
    { icon: FileText,     label: 'Blog Posts',       value: blogPosts.length, color: '#10b981', link: '/admin/blog' },
    { icon: Star,         label: 'Avg. Rating',      value: avgRating,        color: '#f59e0b', link: '/admin/reviews' },
    { icon: Radio,        label: 'Live Visitors',    value: liveNow,          color: '#22c55e', link: '/admin/analytics' },
  ]
