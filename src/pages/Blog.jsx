      ...
      <section style={{ padding: 'clamp(4rem,8vw,7rem) clamp(1.25rem,5vw,4.5rem)', background: 'var(--bg)' }}>
        <div style={{ maxWidth: 1240, margin: '0 auto' }}>
                ...
                <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-xl)', overflow: 'hidden', display: 'grid', gridTemplateColumns: '1fr 1fr', transition: 'all 0.3s' }}
                  ...
                  <div style={{ height: 320, position: 'relative', overflow: 'hidden', background: BG_MAP[filtered[0].category] || '#111' }}>
                    {filtered[0].cover_image
                      ? <img src={filtered[0].cover_image} alt={filtered[0].title} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
                  ...
                  <div style={{ padding: '2.5rem', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                    <span style={{ fontSize: '0.72rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: CAT_CLR[filtered[0].category] || 'var(--red)', marginBottom: '0.75rem', display: 'block' }}>{filtered[0].category} · {filtered[0].read_time}</span>
                  ...
                  <Link to={`/blog/${post.slug}`} style={{ textDecoration: 'none', display: 'block', height: '100%' }}>
                    <article style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', overflow: 'hidden', height: '100%', transition: 'all 0.3s var(--ease)' }}
                      ...
                      <div style={{ height: 200, position: 'relative', overflow: 'hidden', background: BG_MAP[post.category] || '#111' }}>
                        {post.cover_image
                          ? <img src={post.cover_image} alt={post.title} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', transition: 'transform 0.6s ease' }} />
                      ...
                      <div style={{ padding: '1.5rem' }}>
                        <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginBottom: '0.6rem' }}>{post.read_time} · {fmtDate(post.published_at || post.created_at)}</div>
          ...
          {!loading && filtered.length === 0 && (
            <div style={{ textAlign: 'center', padding: '4rem', color: 'var(--text-muted)' }}>No posts in this category yet.</div>
          )}
        </div>
      </section>
    </PageWrapper>
